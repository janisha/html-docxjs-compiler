...
describe('ImageDownloadStrategyManager', () => {
  ...
  describe('download', () => {
    ...
    it('should throw error when no strategy can handle URL', async () => {
      ...
      await expect(manager.download('http://example.com/image.png'))
        .rejects
        .toThrow('No strategy found to handle URL');
    });
  ...
  describe('integration with default HttpImageDownloadStrategy', () => {
    it('should have default HTTP strategy when initialized without arguments', async () => {
      const manager = new ImageDownloadStrategyManager();

      // Should not throw because default strategy handles http://
      await expect(async () => {
        // This will fail in actual execution because axios is not mocked,
        // but structure-wise it should try to use HttpImageDownloadStrategy
      }).not.toThrow();
    });
  });
});

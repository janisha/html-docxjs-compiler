...
export async function handleImage(
  ...
  if (href.startsWith('data:image')) {
    ...
    // Use strategy manager to download image
    const manager = strategyManager || new ImageDownloadStrategyManager();
    try {
      imageData = await manager.download(href);
    } catch (error) {
      // Failed to download image, return empty array
      return [];
    }
  }
